...
class WellKnownTypesSpec extends AnyFlatSpec with Matchers {
  ...
  "duration" should "serialize and parse correctly" in {
    ...
    JsonFormat.parser.fromJsonString[WellKnownTest](durationJson) must be(
      durationProto
    )
  }
  ...
  "Timestamp parser" should "work" in {
    ...
    Timestamps.parseTimestamp("1969-12-31T23:59:59.999Z") must be(
      Timestamp(-1, 999000000)
    )

    Timestamps.writeTimestamp(Timestamp(nanos = 10)) must be(
      "1970-01-01T00:00:00.000000010Z"
    )
    Timestamps.writeTimestamp(Timestamp(nanos = 10000)) must be(
      "1970-01-01T00:00:00.000010Z"
    )
    Timestamps.writeTimestamp(Timestamp(nanos = 10000000)) must be(
      "1970-01-01T00:00:00.010Z"
    )

    Timestamps.writeTimestamp(
      Timestamps.parseTimestamp("1970-01-01T00:00:00.010+08:35")
    ) must be("1969-12-31T15:25:00.010Z")
    Timestamps.writeTimestamp(
      Timestamps.parseTimestamp("1970-01-01T00:00:00.010-08:12")
    ) must be("1970-01-01T08:12:00.010Z")
  }
  ...
  "timestamp" should "serialize and parse correctly" in {
    val timestampJson = """{
                          ...
                          |}""".stripMargin
    val timestampProto = WellKnownTest(
      timestamp = Some(Timestamp(seconds = 1474029324, nanos = 375123456))
    )
    JsonFormat.parser.fromJsonString[WellKnownTest](timestampJson) must be(
      timestampProto
    )
  ...
  val fieldMaskProto = WellKnownTest(
    mask = Some(FieldMask(paths = Seq("a", "b")))
  )
  ...
  "FieldMask" should "print comma separated string" in {
    ...
    JsonFormat.printer.toJson(fieldMaskProto) must be(
      parse(
  ...
  it should "parse comma separated string" in {
    JsonFormat.parser.fromJsonString[WellKnownTest](fieldMaskJson) must be(
      fieldMaskProto
    )
  }
}
