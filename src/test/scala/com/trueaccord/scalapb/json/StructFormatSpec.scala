...
class StructFormatSpec extends FlatSpec with MustMatchers with JavaAssertions {
  ...
  val StructExample = Struct(fields = Map(
  ...
  val StructExample2 = Struct(fields = Map(
    "f1" -> Value(Value.Kind.StringValue("Boo")),
  ...
  "Value" should "be serialized the same as in Java (and parsed back to original)" in {
    ...
    assertJsonIsSameAsJava(Value(kind = Value.Kind.ListValue(com.google.protobuf.struct.ListValue(values = Seq(
  ...
  "NullValue" should "be serialized and parsed from JSON correctly" in {
    ...
    javaParse("""{"repNv": [0,0.0,0]}""", jsontest.Test3.StructTest.newBuilder).toString must be ("rep_nv: NULL_VALUE\n" * 3)
    ...
    JsonFormat.fromJsonString[StructTest]("""{"repNv": [null, 0, null]}""") must be(StructTest(repNv = Seq(
      NullValue.NULL_VALUE, NullValue.NULL_VALUE, NullValue.NULL_VALUE)))
  }
}
